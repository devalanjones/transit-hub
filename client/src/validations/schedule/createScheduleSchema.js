...
const createScheduleSchema = yup.object({
  busId: yup.string().required("Bus is required"),
  ...
  stops: yup
    ...
    .of(
      yup.object({
        ...
        expectedArrivalTime: timeString.label("Expected arrival time"),
      })
    )
    .min(2, "At least 2 stops are required")
    .required("Stops are required"),

  arrivalTime: timeString.label("Arrival time"),
  departureTime: timeString.label("Departure time"),
